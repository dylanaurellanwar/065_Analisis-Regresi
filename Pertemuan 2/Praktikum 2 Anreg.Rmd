...
data <- read_excel("C:/Users/dylan/Downloads/NSC-2025.xlsx", sheet="Sheet2")
...
$\hat{\beta_1} = \frac{\sum(y_i - \bar{y}) - \sum(x_i - \bar{x})}{\sum(x_i - \bar{x})^2} = \frac{S_{xx}}{S_{xx}} atau$
...
dengan Y adalah Indeks Pembangunan Manusia dan X3 Persentase Penduduk Miskin.

Didapatkan nilai $\hat\beta_0 = 84.2552$ artinya nilai dugaan rataan indeks pembangunan manusia ketika persentase penduduk miskin bernilai 0 persen adalah 84.2552%, akan tetapi nilai 0 tidak ada dalam selang pengamatan persentase penduduk miskin sehingga $\hat\beta_0$ tidak dapat didefinisikan lebih lanjut

Nilai $\hat\beta_1 = -0.9377205$ artinya nilai dugaan perubahan dugaan rataan Indeks Pembangunan Manusia jika persentase penduduk miskin berubah satu persen adalah berkurang sebesar 0.9377205%

Koefisien determinasi yang dihasilkan adalah 0.4787224 atau 47.87% artinya keragaman indeks pembangunan manusia yang dapat dijelaskan oleh persentase penduduk miskin adalah sebesar 47.87%, sisanya yaitu 52.13% dijelaskan oleh peubah lain yang tidak dimasukkan dalam model.
